...
const urlConsultas = 'http://localhost:9000/api/eventos/5'
...
class  PageEventos extends Component {
  ...
  peticionPost = async () => {
    ...
    .then(response=>{
      ...
      this.peticionConsultaEventosGet(); /// para actualizar el listado
  ...
  peticionPut = async () => {
    ...
    .then(response => {
      this.modalInsertar();
      this.peticionConsultaEventosGet();
      ...
      /*<div className="App">
        ...
        <br /><br />
          <table className="table ">
            ...
            <tbody>
              {
                this.state.consulta.map(eventos  => {
                  return (
                    <tr key={eventos.sec}>
                      <td>{eventos.sec}</td>
                      <td>{eventos.fecha}</td>
                      <td>{eventos.equi1}</td>
                      <td>{eventos.marca1}</td>
                      <td>{eventos.equi2}</td>
                      <td>{eventos.marca2}</td>
                      <td>{eventos.deporte}</td>
                      <td>{eventos.descrip}</td>
                      <td>
                        {eventos.eve_acciones}
                        <button className='btn btn-primary' onClick={()=>{this.seleccionarEvento(eventos); this.modalInsertar()}}><FontAwesomeIcon icon={faEdit}></FontAwesomeIcon></button>
                        {"  "}
                        <button className='btn btn-danger' onClick={()=>{this.seleccionarEvento(this.state.eventos); this.modalEliminar()}}><FontAwesomeIcon icon={faTrashAlt}></FontAwesomeIcon></button>
                      </td>
                    </tr>
                  )
                })
          ...
          <Modal isOpen={this.state.modalInsertar}>
            ...
            <ModalBody>
              ...
              <input className="form-control" type="text" name="eve_id" readOnly onChange={this.handleChange} value = {form?form.eve_id:this.state.eventos.length+1}></input><br />

              <label htmlFor="eve_fecha">Fecha</label>
              <input className='form-control' type="text" name="eve_fecha" onChange={this.handleChange} value = {form?form.eve_fecha:''}></input><br />

              <label>Equipo1</label>
              <Select
                options = {this.state.equipos.map(equipo => ({label: equipo.equ_nombre, value: equipo.equ_id}) )}
                onChange={ handleSelectChange }
              /><br />

              <label htmlFor='eve_marca1'>Marcador Equipo1</label>
              <input className='form-control' type="text" name="eve_marca1" onChange={this.handleChange} value = {form?form.eve_marca1:''}></input><br />
              ...
              <input className='form-control' type="text" name="eve_marca2" onChange={this.handleChange} value = {form?form.eve_marca2:''}></input><br />

              <label>Deporte</label>
              <Select
                options = {this.state.deportes.map(deporte => ({label: deporte.dep_nombre, value: deporte.dep_id}) )}
                onChange={ handleSelectChange }
              /><br />

              <label htmlFor='eve_descrip'>Descripción</label>
              <input className='form-control' type="text" name="eve_descrip" onChange={this.handleChange} value = {form?form.eve_descrip:''}></input><br />
            </ModalBody>
